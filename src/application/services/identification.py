...
# Абсолютный предел RMS кандидата. Отрыв — величина **относительная**,
# и сам по себе он не отличает «нашли объект» от «не подошёл никто»:
# трек, к которому не подходит ничего, даёт лучший RMS 5 кГц при следующем
# 20 кГц, то есть отрыв ×4, и вердикт выходит уверенным на пустом месте.
#
# `rffit` этой дыры не имеет: клавиша `i` первым делом спрашивает предел
# в кГц, и список кандидатов — это те, кто под него влез. В гайде взяты
# 0.5 кГц для разбора запуска и 0.1 кГц для опознания одиночного сигнала.
# Наши 0.5 совпадают с ними и заодно с порогом go/no-go фазы 0.
#
# **Пустой список после отсечки — это ответ**, а не отсутствие ответа:
# «такого объекта в каталоге нет» (упражнение 6 гайда). UI обязан подать
# его словами, а не как «перебора не было».
#
# Замер на golden-корпусе: по всему каталогу 26 кандидатов → 8, внутри
# запуска 22 → 7, худший из отсечённых — 5.9 кГц.
MAX_CANDIDATE_RMS_KHZ = 0.5
...
def _score(
    ...
    """Кандидаты под абсолютным пределом `MAX_CANDIDATE_RMS_KHZ`, по возрастанию RMS."""
    key = segments[0].key
    ...
    for obj in objects:
        ...
        if result is None:
            ...
            continue
        rms, carriers = result
        if rms > MAX_CANDIDATE_RMS_KHZ:
            # Не «плохой кандидат», а не кандидат вовсе: держать его в списке
            # значит дать отрыву измерять расстояние между двумя промахами.
            continue
        scored.append(
            Candidate(
