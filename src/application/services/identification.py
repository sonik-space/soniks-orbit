...
def is_screenable(track: ObservationTrack) -> bool:
    ...
    Пустой трек в перебор не идёт по построению: извлечение здесь работает
    без человека, и наблюдения, где оно ничего не нашло, просто не попадают
    в идентификацию (decisions/006). Это примерно половина корпуса.

    Ненадёжный — тоже. Запас между лучшей и следующей конвенцией оси частот
    порядка единицы означает, что конвенции неразличимы, то есть неверна
    сама абсолютная частота точек. Показывать кандидатов на таком треке
    бессмысленно: перебор ранжировал бы по шуму, а список кандидатов
    выглядел бы так же убедительно, как настоящий.
    """
    diagnostics = track.diagnostics or {}
    return track.n_points > 0 and bool(diagnostics.get("reliable"))
