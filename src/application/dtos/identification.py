...
class CreateIdentificationRequest(BaseModel):
    ...
    Кнопка нужна оператору и демонстрации, но она **не заменяет автомат**:
    вариант «только по кнопке» отвергнут в decisions/006, поэтому основной
    путь — почасовой сканер, а этот вызов ставит ту же задачу в ту же очередь.
    """
